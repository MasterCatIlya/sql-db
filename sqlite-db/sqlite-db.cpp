...
private:
    ...
    friend void sqlQuery(SQLiteDB& db);
};

// выполнение SQL-запросов
void sqlQuery(SQLiteDB& dbInstance) {
...
int main() {
    setlocale(LC_ALL, "ru");
    std::string dbName;
    std::cout << "Введите название БД с (.db): " << std::endl;
    std::getline(std::cin, dbName);

    SQLiteDB db(dbName);
    if (!db.connectOrCreate()) {
        std::cout << "Не удалось подключиться к БД" << std::endl;
        return -1;
    }
